    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 tracking-tight">Housekeeping Board</h1>
                    <p className="text-slate-500 text-sm mt-1">Manage room turnovers and cleaning cycles.</p>
                </div>
                <button 
                    onClick={fetchRooms}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                        ...
                        <div key={room.id} className="premium-card !bg-white group hover:shadow-lg transition-all duration-300">
                            ...
                            <div className="flex flex-col gap-3 mt-6">
                                ...
                                {room.status === 'CLEANING' && (
                                    ...
                                    >
                                        <CheckCircle size={16} /> Mark as Inspected
                                    </button>
                                )}
                                {room.status === 'INSPECTED' && (
                                    ...
                                    >
                                        <Sparkles size={16} /> Ready for Guests
                                    </button>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="col-span-full py-20 bg-white rounded-2xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800">All Rooms are Clean!</h3>
                        <p className="text-slate-400 text-sm max-w-xs mt-2">There are currently no rooms marked as Dirty or Cleaning in the system.</p>
                    </div>
                )}
            </div>
        </div>
